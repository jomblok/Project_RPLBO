...
public class MainController {
    ...
    public void initialize() {
        ...
        // Listener untuk search by kategori
        searchKategori.textProperty().addListener((observable, oldValue, newValue) -> {
            filterByKategoriTextField(newValue);
        });

        // Listener untuk search by kata kunci (search all fields)
        searchKataKunci.textProperty().addListener((observable, oldValue, newValue) -> {
            filterToDoByAllFields(newValue);
        });
    }
